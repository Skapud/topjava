...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.BETWEEN_HALF_OPEN, query = "SELECT m FROM Meal m LEFT JOIN FETCH m.user " +
                "WHERE m.user.id =:user_id " +
...
@Entity
@Table(name = "meal",
uniqueConstraints = {@UniqueConstraint(columnNames = {"user_id", "date_time"}, name = "uk_userid_datetime")})
